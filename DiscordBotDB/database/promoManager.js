...
function getDB() {
  if (!global.db) throw new Error('global.db не инициализирован. Убедись, что база подключена в index.js');
  return global.db;
}
...
async function createPromoCode(code, rewards, expiresAt, maxUses = 0) {
  ...
  await getDB().set(promoKey(data.code), data);
  return true;
}

async function getPromoCode(code) {
  return getDB().get(promoKey(code));
}
