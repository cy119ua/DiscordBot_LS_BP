...
const handlers = {
  ...
  ddcuplock: {
    adminOnly: true,
    async run(interaction) {
      console.log(`[ddcuplock] handler invoked, interaction.id=${interaction.id}`);
      ...
      console.log(`[ddcuplock] action parsed: ${action}`);
      if (action !== 'lock' && action !== 'unlock') {
        console.log(`[ddcuplock] invalid action="${action}", replying with error`);
      ...
      console.log(`[ddcuplock] action="${action}" is valid, calling patchSettings`);
      try {
        ...
        console.log(`[ddcuplock] patchSettings completed successfully`);
      } catch (e) {
  ...
  userstat: {
    ...
    async run(interaction) {
      ...
      const lines = users.map((u, i) => {
        ...
        return `${i + 1}. ${star}<@${u.id}> — Уровень: ${u.level} — Паки карт: ${u.cardPacks}`;
      });
